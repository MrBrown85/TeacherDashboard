...
window.MCardWidgets = (function () {
  ...
  _renderers.hero = function (st, cid, data) {
    ...
    return (
      ...
      (badges
        ? '<div class="m-scard-badges">' + badges + flagIcon + '</div>'
        : flagIcon
          ? '<div class="m-scard-badges">' + flagIcon + '</div>'
          : '') +
      '</div>' +
...
_renderers.growth = function (st, cid) {
  ...
  var summative = allScores.filter(function (s) {
    return s.type === 'summative' && s.score > 0;
  });
  if (summative.length === 0) return '';

  summative.sort(function (a, b) {
...
_renderers.dispositions = function (st, cid, data) {
  ...
  var size = 48,
    ...
    maxR = 20;
  var svg =
    '<svg class="m-wdg-petal" width="' + size + '" height="' + size + '" viewBox="0 0 ' + size + ' ' + size + '">';
  ...
  svg +=
    '<polygon points="' + points.join(' ') + '" fill="var(--active-light)" stroke="var(--active)" stroke-width="1.5"/>';
  svg += '</svg>';

  // Find top 2 dimensions
  var indexed = OBS_DIMS.map(function (d, i) {
    return { dim: d, val: vals[i] };
  });
  indexed.sort(function (a, b) {
    return b.val - a.val;
  });
  var top2 = indexed.slice(0, 2).map(function (x) {
    return OBS_LABELS[x.dim];
  });
  var summaryText = 'Strong in ' + top2.join(', ');

  return '<div class="m-wdg-dispositions">' + svg + '<div class="m-wdg-disp-text">' + summaryText + '</div>' + '</div>';
};
...
function _petalPath(sides, radius, cx, cy, fill, stroke) {
  ...
  return (
    '<polygon points="' + pts.join(' ') + '" fill="' + fill + '" stroke="' + (stroke || 'none') + '" stroke-width="1"/>'
  );
}
```
...
_renderers.traits = function (st, cid, data) {
  ...
  var positive = rating.socialTraits.filter(function (t) {
    return SOCIAL_TRAITS_POSITIVE_IDS.has(t);
  });
  ...
  positive.slice(0, maxShow).forEach(function (tid) {
    var trait = SOCIAL_TRAITS_POSITIVE.find(function (t) {
      return t.id === tid;
    });
...
_renderers.concerns = function (st, cid, data) {
  ...
  var concern = rating.socialTraits.filter(function (t) {
    return SOCIAL_TRAITS_CONCERN_IDS.has(t);
  });
  ...
  concern.slice(0, maxShow).forEach(function (tid) {
    var trait = SOCIAL_TRAITS_CONCERN.find(function (t) {
      return t.id === tid;
    });
    var label = trait ? trait.label : tid;
...
describe('Widget Renderers — Remaining', () => {
  ...
  it('growthAreas renders tag chips with section dots', () => {
    ...
    localStorage.setItem('gb-sections-course-1', JSON.stringify([{ id: 'sec1', name: 'Number', color: '#4A90D9' }]));
      ...
      JSON.stringify({
        s1: {
          'term-1': {
            dims: {},
            narrative:
              '<p>Cece has demonstrated outstanding growth in mathematical reasoning this term. She approaches challenges with curiosity.</p>',
          },
        },
      }),
    );
...
_renderers.workHabits = function (st, cid, data) {
  ...
  function _pips(val, max) {
    ...
    for (var i = 1; i <= max; i++) {
      ...
      html +=
        '<div class="m-wdg-pip' + (filled ? ' m-wdg-pip-filled' : '') + '" style="background:' + color + '"></div>';
    }
    return html;
  }
...
_renderers.growthAreas = function (st, cid, data) {
  ...
  rating.growthAreas.slice(0, maxShow).forEach(function (tid) {
    ...
    chips +=
      '<span class="m-wdg-chip m-wdg-chip-neutral">' +
      '<span class="m-wdg-chip-dot" style="background:' +
      dotColor +
      '"></span>' +
      MC.esc(label) +
      '</span>';
...
.m-scard-widgets::-webkit-scrollbar {
  display: none;
}

/* Scroll fade indicator at bottom of widget zone */
.m-scard-widgets::after {
  content: '';
  position: sticky;
  bottom: 0;
  left: 0;
...
window.MCardWidgetEditor = (function () {
  ...
  function buildEditorHTML() {
    ...
    config.order.forEach(function (key) {
      var w = WIDGET_REGISTRY.find(function (r) {
        return r.key === key;
      });
      if (!w) return;
  ...
  function _editorRow(widget, enabled, noDrag) {
    return (
      '<div class="m-wdg-editor-row" data-widget="' +
      widget.key +
      '" data-enabled="' +
      enabled +
      '">' +
      (noDrag
        ? '<div class="m-wdg-editor-drag-spacer"></div>'
        : '<div class="m-wdg-editor-drag" data-action="m-wdg-drag">\u2630</div>') +
      '<div class="m-wdg-editor-label">' +
      widget.label +
      '</div>' +
      '<button class="m-wdg-editor-toggle' +
      (enabled ? ' m-wdg-toggle-on' : '') +
      '" data-action="m-wdg-toggle" data-widget="' +
      widget.key +
      '">' +
...
.m-wdg-editor-row[data-enabled='false'] {
  opacity: 0.5;
}
...
.m-wdg-editor-toggle.m-wdg-toggle-on {
...
.m-wdg-toggle-on .m-wdg-toggle-knob {
