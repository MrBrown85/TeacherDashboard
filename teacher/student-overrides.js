...
window.StudentOverrides = (function() {
  ...
  function toggleOverridePanel(opts) {
    ...
    var html = '<div class="override-panel-header">' +
      ...
      '<button class="override-level-btn' + (getSelectedLevel() === 4 ? ' selected' : '') + '" data-level="4" data-action="selectOverrideLevel" data-value="4">4 Extending</button>' +
      '<button class="override-level-btn' + (getSelectedLevel() === 3 ? ' selected' : '') + '" data-level="3" data-action="selectOverrideLevel" data-value="3">3 Proficient</button>' +
      '<button class="override-level-btn' + (getSelectedLevel() === 2 ? ' selected' : '') + '" data-level="2" data-action="selectOverrideLevel" data-value="2">2 Developing</button>' +
      '<button class="override-level-btn' + (getSelectedLevel() === 1 ? ' selected' : '') + '" data-level="1" data-action="selectOverrideLevel" data-value="1">1 Emerging</button>' +
      '</div>' +
  ...
  function saveOverride(activeCourse, studentId, secId, selectedLevel, onSaved) {
    ...
    if (selectedLevel === 0) { alert('Select a proficiency level.'); return false; }
    if (!reason) {
      if (reasonEl) {
